"""
Human Ruling E2E Test Task for goal-dfd7206a7b8d
...
SQLALCHEMY_DATABASE_URL = "sqlite:///./test_human_ruling_dfd7206a7b8d.db"
...
def test_verification_fails_3_times_triggers_disputed():
    """
    Test that verification failing 3 times triggers the disputed state for goal-dfd7206a7b8d
    ...
    """
    print("\n" + "="*70)
    print("HUMAN RULING E2E TEST FOR GOAL-DFD7206A7B8D")
    ...
    try:
        ...
        # Create goal with specific ID for goal-dfd7206a7b8d
        goal_id = "goal-dfd7206a7b8d"
        ...
        if not existing_goal:
            ...
            """), {
                "id": goal_id,
                "title": "Human Ruling Test Goal for DFD7206A7B8D",
                "description": "Goal for testing disputed state trigger for goal-dfd7206a7b8d",
                "status": "active",
                "verifier_agent_id": "human-ruling-verifier-dfd7206a7b8d",
        ...
        # Create project
        project_id = f"proj-dfd7206a7b8d-{uuid.uuid4().hex[:8]}"
        db.execute(text("""
        ...
        """), {
            "id": project_id,
            "name": "Human Ruling Test Project for DFD7206A7B8D",
            "description": "Project for testing disputed state for goal-dfd7206a7b8d",
            "status": "active",
            "goal_id": goal_id,
            "created_at": datetime.now(),
            "updated_at": datetime.now()
        ...
        task_id = f"task-disputed-dfd7206a7b8d-{uuid.uuid4().hex[:8]}"
        acceptance_criteria = {
            "criteria": [
                {
                    "type": "compile",
                    "name": "Compilation Check",
                    "desc": "Verify code compiles successfully for goal-dfd7206a7b8d"
                },
                {
                    "type": "api",
                    "endpoint": "http://localhost:8090/api/v1/health",
                    "name": "API Health Check",
                    "desc": "Verify API is healthy for goal-dfd7206a7b8d"
                }
            ]
        }
        ...
        """), {
            "id": task_id,
            "title": "Human Ruling Test Task for goal-dfd7206a7b8d - Will Trigger Disputed",
            "description": "This task will fail verification 3 times to trigger disputed state for goal-dfd7206a7b8d",
            "status": "in_progress",
            "priority": "high",
            "project_id": project_id,
            "goal_id": goal_id,
            "assigned_agent": "test-agent-dfd7206a7b8d",
        ...
        def mock_run_checks_always_fail(task_id, result):
            ...
            return False, "Verification failed - compilation error detected (mocked for goal-dfd7206a7b8d)"
        ...
        print("\n" + "="*70)
        print("✅ HUMAN RULING E2E TEST FOR GOAL-DFD7206A7B8D PASSED")
        print("="*70)
        print("Summary:")
        ...
        print(f"\n❌ HUMAN RULING E2E TEST FOR GOAL-DFD7206A7B8D FAILED: {e}")
        import traceback
        traceback.print_exc()
        raise e
...
def test_verification_passes_after_retry():
    """
    Test that if the third attempt passes, the task goes to done for goal-dfd7206a7b8d
    """
    print("\n" + "="*70)
    print("HUMAN RULING E2E TEST - PASSES ON THIRD ATTEMPT FOR GOAL-DFD7206A7B8D")
    print("="*70)
    ...
    try:
        ...
        goal_id = "goal-dfd7206a7b8d-pass"
        db.execute(text("""
            INSERT INTO goals (id, title, description, status, verifier_agent_id, created_at, updated_at)
            VALUES (:id, :title, :description, :status, :verifier_agent_id, :created_at, :updated_at)
        """), {
            "id": goal_id,
            "title": "Human Ruling Test Goal for DFD7206A7B8D - Pass",
            "description": "Goal for testing passing verification for goal-dfd7206a7b8d",
            "status": "active",
            "verifier_agent_id": "human-ruling-verifier-dfd7206a7b8d",
            "created_at": datetime.now(),
            "updated_at": datetime.now()
        ...
        # Create project
        project_id = f"proj-pass-dfd7206a7b8d-{uuid.uuid4().hex[:8]}"
        db.execute(text("""
            INSERT INTO projects (id, name, description, status, goal_id, created_at, updated_at, verifier_agent_id)
            VALUES (:id, :name, :description, :status, :goal_id, :created_at, :updated_at, NULL)
        """), {
            "id": project_id,
            "name": "Human Ruling Test Project for DFD7206A7B8D - Pass",
            "description": "Project for testing passing verification for goal-dfd7206a7b8d",
            "status": "active",
            "goal_id": goal_id,
            "created_at": datetime.now(),
            "updated_at": datetime.now()
        ...
        # Create task
        task_id = f"task-pass-dfd7206a7b8d-{uuid.uuid4().hex[:8]}"
        db.execute(text("""
            INSERT INTO tasks (id, title, description, status, priority, project_id, goal_id, 
                            assigned_agent, verification_cycle, acceptance_criteria, created_at, updated_at, verifier_agent_id)
            VALUES (:id, :title, :description, :status, :priority, :project_id, :goal_id, 
        ...
        """), {
            "id": task_id,
            "title": "Human Ruling Test Task for goal-dfd7206a7b8d - Passes on Third",
            "description": "This task passes on third attempt for goal-dfd7206a7b8d",
            "status": "in_progress",
            "priority": "high",
            "project_id": project_id,
            "goal_id": goal_id,
            "assigned_agent": "test-agent-dfd7206a7b8d",
            "verification_cycle": 0,
            "acceptance_criteria": json.dumps({"criteria": [{"type": "compile", "name": "Check", "desc": "Verify for goal-dfd7206a7b8d"}]}),
            "created_at": datetime.now(),
            "updated_at": datetime.now()
        })
        db.commit()
        ...
        def mock_fail_then_pass(task_id, result):
            call_count[0] += 1
            if call_count[0] < 3:
                return False, f"Verification failed (attempt {call_count[0]}) for goal-dfd7206a7b8d"
            return True, "All checks passed on third attempt for goal-dfd7206a7b8d"
        
        original_run_checks = verifier._run_verifier_checks
        verifier._run_verifier_checks = mock_fail_then_pass
        
        # First attempt (fail)
        result1 = verifier.verify(task_id, "Failed first attempt for goal-dfd7206a7b8d", success=True)
        ...
        # Second attempt (fail)
        result2 = verifier.verify(task_id, "Failed second attempt for goal-dfd7206a7b8d", success=True)
        print(f"✓ Second attempt: {result2['action']}")
        task_status = db.execute(text("SELECT status FROM tasks WHERE id = :id"), {"id": task_id}).fetchone()
        assert task_status[0] == "review_needed"
        
        # Third attempt (pass!)
        result3 = verifier.verify(task_id, "Passed on third attempt for goal-dfd7206a7b8d", success=True)
        ...
        print("\n✅ Test passed: Task goes to done after third successful attempt for goal-dfd7206a7b8d")
        return True
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\n" + "="*70)
        print("TEST 1: FAILED 3 TIMES → DISPUTED FOR GOAL-DFD7206A7B8D")
        print("="*70)
    except Exception as e:
        success = False
        print(f"\n❌ TEST 1 FAILED: {e}")
    ...
    try:
        ...
        print("\n" + "="*70)
        print("TEST 2: FAILS TWICE, PASSES THIRD → DONE FOR GOAL-DFD7206A7B8D")
        print("="*70)
    except Exception as e:
        success = False
        print(f"\n❌ TEST 2 FAILED: {e}")
    ...
    if success:
        print("\n" + "="*70)
        print("🎉 ALL HUMAN RULING E2E TESTS FOR GOAL-DFD7206A7B8D PASSED!")
        print("="*70)
    else:
        print("\n" + "="*70)
        print("❌ SOME TESTS FAILED FOR GOAL-DFD7206A7B8D")
        print("="*70)
        exit(1)