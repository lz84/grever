"""
Human Ruling E2E Test Task for goal-a5b18e1945b0
...
SQLALCHEMY_DATABASE_URL = "sqlite:///./test_human_ruling_a5b18e1945b0.db"
...
def test_verification_fails_3_times_triggers_disputed():
    """
    Test that verification failing 3 times triggers the disputed state for goal-a5b18e1945b0
    ...
    """
    print("\n" + "="*70)
    print("HUMAN RULING E2E TEST FOR GOAL-A5B18E1945B0")
    ...
    try:
        ...
        # Create goal with specific ID for goal-a5b18e1945b0
        goal_id = "goal-a5b18e1945b0"
        ...
        if not existing_goal:
            ...
            """), {
                "id": goal_id,
                "title": "Human Ruling Test Goal for A5B18E1945B0",
                "description": "Goal for testing disputed state trigger for goal-a5b18e1945b0",
                "status": "active",
                "verifier_agent_id": "human-ruling-verifier-a5b18e1945b0",
        ...
        # Create project
        project_id = f"proj-a5b18e1945b0-{uuid.uuid4().hex[:8]}"
        db.execute(text("""
        ...
        """), {
            "id": project_id,
            "name": "Human Ruling Test Project for A5B18E1945B0",
            "description": "Project for testing disputed state for goal-a5b18e1945b0",
            "status": "active",
            "goal_id": goal_id,
            "created_at": datetime.now(),
            "updated_at": datetime.now()
        ...
        task_id = f"task-disputed-a5b18e1945b0-{uuid.uuid4().hex[:8]}"
        acceptance_criteria = {
            "criteria": [
                {
                    "type": "compile",
                    "name": "Compilation Check",
                    "desc": "Verify code compiles successfully for goal-a5b18e1945b0"
                },
                {
                    "type": "api",
                    "endpoint": "http://localhost:8090/api/v1/health",
                    "name": "API Health Check",
                    "desc": "Verify API is healthy for goal-a5b18e1945b0"
                }
            ]
        }
        ...
        """), {
            "id": task_id,
            "title": "Human Ruling Test Task for goal-a5b18e1945b0 - Will Trigger Disputed",
            "description": "This task will fail verification 3 times to trigger disputed state for goal-a5b18e1945b0",
            "status": "in_progress",
            "priority": "high",
            "project_id": project_id,
            "goal_id": goal_id,
            "assigned_agent": "test-agent-a5b18e1945b0",
        ...
        def mock_run_checks_always_fail(task_id, result):
            ...
            return False, "Verification failed - compilation error detected (mocked for goal-a5b18e1945b0)"
        ...
        print("\n" + "="*70)
        print("✅ HUMAN RULING E2E TEST FOR GOAL-A5B18E1945B0 PASSED")
        print("="*70)
        print("Summary:")
        ...
        print(f"\n❌ HUMAN RULING E2E TEST FOR GOAL-A5B18E1945B0 FAILED: {e}")
        import traceback
        traceback.print_exc()
        raise e
...
def test_verification_passes_after_retry():
    """
    Test that if the third attempt passes, the task goes to done for goal-a5b18e1945b0
    """
    print("\n" + "="*70)
    print("HUMAN RULING E2E TEST - PASSES ON THIRD ATTEMPT FOR GOAL-A5B18E1945B0")
    print("="*70)
    ...
    try:
        ...
        goal_id = "goal-a5b18e1945b0-pass"
        db.execute(text("""
            INSERT INTO goals (id, title, description, status, verifier_agent_id, created_at, updated_at)
            VALUES (:id, :title, :description, :status, :verifier_agent_id, :created_at, :updated_at)
        """), {
            "id": goal_id,
            "title": "Human Ruling Test Goal for A5B18E1945B0 - Pass",
            "description": "Goal for testing passing verification for goal-a5b18e1945b0",
            "status": "active",
            "verifier_agent_id": "human-ruling-verifier-a5b18e1945b0",
            "created_at": datetime.now(),
            "updated_at": datetime.now()
        ...
        # Create project
        project_id = f"proj-pass-a5b18e1945b0-{uuid.uuid4().hex[:8]}"
        db.execute(text("""
            INSERT INTO projects (id, name, description, status, goal_id, created_at, updated_at, verifier_agent_id)
            VALUES (:id, :name, :description, :status, :goal_id, :created_at, :updated_at, NULL)
        """), {
            "id": project_id,
            "name": "Human Ruling Test Project for A5B18E1945B0 - Pass",
            "description": "Project for testing passing verification for goal-a5b18e1945b0",
            "status": "active",
            "goal_id": goal_id,
            "created_at": datetime.now(),
            "updated_at": datetime.now()
        ...
        # Create task
        task_id = f"task-pass-a5b18e1945b0-{uuid.uuid4().hex[:8]}"
        db.execute(text("""
            INSERT INTO tasks (id, title, description, status, priority, project_id, goal_id, 
                            assigned_agent, verification_cycle, acceptance_criteria, created_at, updated_at, verifier_agent_id)
            VALUES (:id, :title, :description, :status, :priority, :project_id, :goal_id, 
        ...
        """), {
            "id": task_id,
            "title": "Human Ruling Test Task for goal-a5b18e1945b0 - Passes on Third",
            "description": "This task passes on third attempt for goal-a5b18e1945b0",
            "status": "in_progress",
            "priority": "high",
            "project_id": project_id,
            "goal_id": goal_id,
            "assigned_agent": "test-agent-a5b18e1945b0",
            "verification_cycle": 0,
            "acceptance_criteria": json.dumps({"criteria": [{"type": "compile", "name": "Check", "desc": "Verify for goal-a5b18e1945b0"}]}),
            "created_at": datetime.now(),
            "updated_at": datetime.now()
        })
        db.commit()
        ...
        def mock_fail_then_pass(task_id, result):
            call_count[0] += 1
            if call_count[0] < 3:
                return False, f"Verification failed (attempt {call_count[0]}) for goal-a5b18e1945b0"
            return True, "All checks passed on third attempt for goal-a5b18e1945b0"
        
        original_run_checks = verifier._run_verifier_checks
        verifier._run_verifier_checks = mock_fail_then_pass
        
        # First attempt (fail)
        result1 = verifier.verify(task_id, "Failed first attempt for goal-a5b18e1945b0", success=True)
        ...
        # Second attempt (fail)
        result2 = verifier.verify(task_id, "Failed second attempt for goal-a5b18e1945b0", success=True)
        print(f"✓ Second attempt: {result2['action']}")
        task_status = db.execute(text("SELECT status FROM tasks WHERE id = :id"), {"id": task_id}).fetchone()
        assert task_status[0] == "review_needed"
        
        # Third attempt (pass!)
        result3 = verifier.verify(task_id, "Passed on third attempt for goal-a5b18e1945b0", success=True)
        ...
        print("\n✅ Test passed: Task goes to done after third successful attempt for goal-a5b18e1945b0")
        return True
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\n" + "="*70)
        print("TEST 1: FAILED 3 TIMES → DISPUTED FOR GOAL-A5B18E1945B0")
        print("="*70)
    except Exception as e:
        success = False
        print(f"\n❌ TEST 1 FAILED: {e}")
    ...
    try:
        ...
        print("\n" + "="*70)
        print("TEST 2: FAILS TWICE, PASSES THIRD → DONE FOR GOAL-A5B18E1945B0")
        print("="*70)
    except Exception as e:
        success = False
        print(f"\n❌ TEST 2 FAILED: {e}")
    ...
    if success:
        print("\n" + "="*70)
        print("🎉 ALL HUMAN RULING E2E TESTS FOR GOAL-A5B18E1945B0 PASSED!")
        print("="*70)
    else:
        print("\n" + "="*70)
        print("❌ SOME TESTS FAILED FOR GOAL-A5B18E1945B0")
        print("="*70)
        exit(1)