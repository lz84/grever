...
SQLALCHEMY_DATABASE_URL = "sqlite:///./test_invalid_endpoint_d6480b66ea75.db"
...
def test_task_with_invalid_api_endpoint():
    ...
    try:
        ...
        """), {
            "id": goal_id,
            "title": "Invalid Endpoint Test Goal for D6480B66EA75",
            "description": "Goal for testing invalid API endpoint in acceptance criteria for goal-d6480b66ea75",
            "status": "active",
        ...
        # Create project
        project_id = f"proj-d6480b66ea75-{uuid.uuid4().hex[:8]}"
        db.execute(text("""
        ...
        """), {
            "id": project_id,
            "name": "Invalid Endpoint Test Project for D6480B66EA75",
            "description": "Project for testing invalid endpoint for goal-d6480b66ea75",
            "status": "active",
            "goal_id": goal_id,
            "created_at": datetime.now(),
            "updated_at": datetime.now()
        ...
        task_id = f"task-invalid-d6480b66ea75-{uuid.uuid4().hex[:8]}"
        ...
        acceptance_criteria = {
            "criteria": [
                {
                    "type": "api",
                    "endpoint": "http://nonexistent-api-endpoint-d6480b66ea75.com/api/health",
                    "name": "Health Check",
                    "desc": "This API endpoint does not exist and will fail verification for goal-d6480b66ea75"
                },
                {
                    "type": "api", 
                    "endpoint": "http://localhost:99999/nonexistent-service",
                    "name": "Port Check",
                    "desc": "Invalid port that will fail for goal-d6480b66ea75"
                }
            ]
        }
        ...
        """), {
            "id": task_id,
            "title": "Task with Invalid API Endpoint for goal-d6480b66ea75",
            "description": "This task has acceptance criteria with invalid API endpoints for goal-d6480b66ea75",
            "status": "in_progress",
            "priority": "high",
        ...
        print(f"    1. http://nonexistent-api-endpoint-d6480b66ea75.com/api/health")
        print(f"    2. http://localhost:99999/nonexistent-service")
        
        # Complete the task
        result = "Task completed with invalid API endpoints in acceptance criteria for goal-d6480b66ea75"
        ...
        print("  ✓ Test specific to goal-d6480b66ea75")
        print("="*70)
...
def test_task_with_valid_api_endpoint_comparison():
    ...
    try:
        ...
        goal_id = "goal-d6480b66ea75-valid"
        db.execute(text("""
            INSERT INTO goals (id, title, description, status, verifier_agent_id, created_at, updated_at)
            VALUES (:id, :title, :description, :status, :verifier_agent_id, :created_at, :updated_at)
        """), {
            "id": goal_id,
            "title": "Valid Endpoint Comparison Goal for D6480B66EA75",
            "description": "Goal for testing valid API endpoint comparison for goal-d6480b66ea75",
            "status": "active",
            "verifier_agent_id": "test-verifier",
            "created_at": datetime.now(),
            "updated_at": datetime.now()
        ...
        # Create project
        project_id = f"proj-d6480b66ea75-valid-{uuid.uuid4().hex[:8]}"
        db.execute(text("""
            INSERT INTO projects (id, name, description, status, goal_id, created_at, updated_at, verifier_agent_id)
            VALUES (:id, :name, :description, :status, :goal_id, :created_at, :updated_at, NULL)
        """), {
            "id": project_id,
            "name": "Valid Endpoint Comparison Project for D6480B66EA75",
            "description": "Project for testing valid endpoint comparison for goal-d6480b66ea75",
            "status": "active",
            "goal_id": goal_id,
            "created_at": datetime.now(),
            "updated_at": datetime.now()
        ...
        task_id = f"task-valid-d6480b66ea75-{uuid.uuid4().hex[:8]}"
            ...
            "criteria": [
                {
                    ...
                    "name": "HTTP Test",
                    "desc": "Valid endpoint that should pass verification for goal-d6480b66ea75"
                }
            ]
        }
        ...
        """), {
            "id": task_id,
            "title": "Task with Valid API Endpoint for goal-d6480b66ea75",
            "description": "This task has acceptance criteria with valid API endpoints for goal-d6480b66ea75",
            "status": "in_progress",
            "priority": "medium",
            "project_id": project_id,
            "goal_id": goal_id,
        ...
        # Complete the task
        result = "Task completed with valid API endpoints in acceptance criteria for goal-d6480b66ea75"
...
if __name__ == "__main__":
    print("="*70)
    print("INVALID API ENDPOINT TEST FOR GOAL-D6480B66EA75")
    print("Goal: goal-d6480b66ea75")
    print("="*70)
    ...
    try:
        test_task_with_invalid_api_endpoint()
    except Exception as e:
        success = False
    ...
    try:
        test_task_with_valid_api_endpoint_comparison()
    except Exception as e:
        # This might fail if httpbin.org is not reachable, but that's acceptable
        print(f"\n⚠️  Valid endpoint test encountered issues (acceptable): {e}")
