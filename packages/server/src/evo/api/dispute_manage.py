...
def raise_dispute(req: DisputeCreate):
    """发起争议"""
    return _get_logic().raise_dispute(req)

@router.get("/", response_model=List[DisputeResponse])
def list_disputes(status: Optional[str] = None, goal_id: Optional[str] = None):
    """列出争议"""
    return _get_logic().list_disputes(status, goal_id)

@router.get("/stats", response_model=DisputeStatsResponse)
def get_dispute_stats():
    """争议统计"""
    return _get_logic().get_dispute_stats()

@router.get("/{dispute_id}", response_model=DisputeResponse)
def get_dispute(dispute_id: str):
    """获取争议"""
    return _get_logic().get_dispute(dispute_id)

@router.patch("/{dispute_id}/resolve")
def resolve_dispute(dispute_id: str, resolution: str, resolved_by: Optional[str] = None):
    """解决争议"""
    return _get_logic().resolve_dispute(dispute_id, resolution, resolved_by)

@router.post("/{dispute_id}/discuss")
def add_discussion(dispute_id: str, req: DiscussRequest):
    """Agent 添加讨论消息"""
    return _get_logic().add_discussion(dispute_id, req)

@router.patch("/{dispute_id}/status")
def update_dispute_status(dispute_id: str, req: UpdateStatusRequest):
    """更新争议状态"""
    return _get_logic().update_dispute_status(dispute_id, req)

@router.post("/{dispute_id}/arbitrate")
def arbitrate_dispute(dispute_id: str, req: ArbitrateRequest):
    """人类仲裁"""
    return _get_logic().arbitrate_dispute(dispute_id, req)

@router.get("/{dispute_id}/timeline", response_model=TimelineResponse)
def get_dispute_timeline(dispute_id: str):
    """获取争议讨论时间线"""
    return _get_logic().get_dispute_timeline(dispute_id)

@router.get("/{dispute_id}/detail", response_model=DisputeDetailResponse)
def get_dispute_detail(dispute_id: str):
    """获取争议详情（含讨论计数）"""
    return _get_logic().get_dispute_detail(dispute_id)