  ...
  "inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2",
  {
    variants: {
      variant: {
        ...
        busy:  "border-transparent bg-orange-100 text-orange-700 hover:bg-orange-100/80",
      },
    },
    defaultVariants: {
      variant: "default",
...
function Badge({ className, variant, ...props }: BadgeProps) {
  // 当传递了自定义 className 时，不自动应用 variant 默认背景色，只应用 base 样式
  if (className && (variant === undefined || variant === 'default')) {
    return (
