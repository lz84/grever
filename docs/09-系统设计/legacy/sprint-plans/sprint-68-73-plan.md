# Sprint 68-73 规划：迭代优化能力

> 基线：`v2.0-verification`（Sprint 67 完成，验证流程闭环）  
> 来源：`docs/evolution/下一阶段进化方向-迭代优化.md`  
> 目标：从"瀑布式任务管理"进化到"迭代优化+方案择优"  
> 原则：每个 Sprint = 前端 + 后端 + 集成 + 验证，不按层拆分

---

## Sprint 68：方案库底层（DB + API + 探索模式字段）
...
### 前端
- **Goal 创建/编辑表单**：新增探索模式选项（模式选择、优化目标、收敛阈值、最大轮次）
- **Project 编辑**：新增模式选项（默认继承 goal，可手动覆盖）
- **方案列表页骨架**：`/goals/{id}/solutions` 路由 + 基础布局 + 空状态
...
- **方案命名规则**：`方案-{round}-{目标名简写}`

### 前端
- **方案列表页完整实现**（`/goals/{id}/solutions`）：
  - 表格：轮次、方案名、状态、核心参数、评分、创建时间
  - 状态标签：✅ 达标（绿色）、❌ 不达标（红色）、🏆 最优（金色）、⛔ 否决（灰色）
  - 点击方案名 → 弹出详情 Modal
- **方案详情 Modal**：完整参数、关联任务列表、操作按钮（标记最优、否决、删除）
...
### 前端
- **迭代状态指示器**（GoalDetail 页面顶部）：当前轮次、状态、最新比较摘要
- **收敛控制按钮**：继续迭代 / 宣布收敛 / 暂停迭代 / 换方向

### 集成验证
- 连续完成 3 个任务 → 每次自动触发比较 → 第 3 次检测到改进 < 5% → waiting_human
- 人类确认"收敛" → 状态变为 converged

---

## Sprint 71：约束调整 + 迭代控制 UI

**一句话**：Nexus 能根据上一轮结果自动调整约束，前端能控制迭代

### 后端
- **约束自动调整逻辑**：
  - 最短工期模式：上一轮工期 × 0.9 作为新上限
  - 最低成本模式：上一轮成本 × 0.9 作为新上限
  - 综合最优模式：所有维度各收紧 5%
- **约束注入 Prompt**：重新派发时将新约束注入任务描述
- **API 端点**：
  - `POST /api/v1/goals/{id}/iterate` — 触发下一轮迭代
  - `GET /api/v1/goals/{id}/constraints` — 查看约束历史

### 前端
- **约束历史面板**（GoalDetail 新增 Tab）：时间线展示每轮约束调整
- **迭代控制面板**：优化目标选择器、最大轮次输入、改进阈值输入
- **方案对比快速预览**（嵌入 GoalDetail 底部）：最近 2 方案的核心维度对比

### 集成验证
- 完成方案A → 点击"继续迭代" → 自动创建新约束 → 新任务派发带约束描述

---

## Sprint 72：方案对比视图（SolutionCenter 页面）

**一句话**：独立的方案对比页面，多维度横向对比 + 收敛趋势图

### 前端
- **SolutionCenter 页面**（`/solutions`）：
  - 目标筛选器 → 加载该目标下所有方案
  - 多维度对比表格（横向方案、纵向维度、最优值高亮）
  - 收敛趋势图（ECharts/recharts 折线图）
  - 方案详情抽屉：完整参数、关联任务、执行日志
- **后端 API**：
  - `GET /api/v1/solutions/compare/multi?goal_id=xxx` — 多维度对比数据
  - `GET /api/v1/solutions/trend?goal_id=xxx` — 收敛趋势数据

### 集成验证
- 打开 SolutionCenter → 选择目标 → 看到对比表格和趋势图

---

## Sprint 73：端到端闭环 + 大桥场景演示
...
```
Sprint 68（DB + API + 探索模式字段）
    ↓
Sprint 69（自动捕获 + 方案列表页）
    ↓
Sprint 70（迭代决策回路 + 比较引擎 + 人类确认节点）
    ↓
Sprint 71（约束调整 + 迭代控制 UI）
    ↓
Sprint 72（SolutionCenter 对比页面）
    ↓
Sprint 73（端到端闭环 + 大桥场景演示）
```
